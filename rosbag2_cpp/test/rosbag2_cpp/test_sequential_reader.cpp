...
public:
  ...
  {
    topic_with_type_.name = "topic";
    topic_with_type_.type = "test_msgs/BasicTypes";
    topic_with_type_.serialization_format = storage_serialization_format_;
  ...
  std::unique_ptr<rosbag2_storage::StorageFactoryInterface> make_storage_mock()
  {
    ...
    ON_CALL(*storage_factory, open_read_only).WillByDefault(
      [this](const rosbag2_storage::StorageOptions & storage_options) {
        EXPECT_STREQ(relative_file_path_.c_str(), storage_options.uri.c_str());
        return storage_;
      });
  ...
  std::unique_ptr<rosbag2_storage::MetadataIo> make_metadataio_mock()
  {
    ...
    metadata.relative_file_paths = {relative_file_path_};
...
TEST_F(SequentialReaderTest, read_next_uses_converters_to_convert_serialization_format) {
  ...
  auto converter_options = rosbag2_cpp::ConverterOptions{"", output_format};
  auto sequential_reader = std::make_shared<rosbag2_cpp::readers::SequentialReader>(
    default_storage_options_, converter_options,
    make_storage_mock(), converter_factory_, make_metadataio_mock());
  auto reader_ = std::make_unique<rosbag2_cpp::Reader>(std::move(sequential_reader));
  reader_->read_next();
}
...
TEST_F(SequentialReaderTest, open_throws_error_if_converter_plugin_does_not_exist) {
  ...
  auto converter_options = rosbag2_cpp::ConverterOptions{"", output_format};
  EXPECT_ANY_THROW(std::make_shared<rosbag2_cpp::readers::SequentialReader>(
    default_storage_options_, converter_options,
    make_storage_mock(), converter_factory_, make_metadataio_mock()));
}
...
TEST_F(
  ...
  auto converter_options = rosbag2_cpp::ConverterOptions{"", storage_serialization_format};
  auto sequential_reader = std::make_shared<rosbag2_cpp::readers::SequentialReader>(
    default_storage_options_, converter_options,
    make_storage_mock(), converter_factory_, make_metadataio_mock());
  auto reader_ = std::make_unique<rosbag2_cpp::Reader>(std::move(sequential_reader));
...
TEST_F(SequentialReaderTest, set_filter_calls_storage) {
  auto converter_options = rosbag2_cpp::ConverterOptions{"", storage_serialization_format_};
  auto sequential_reader = std::make_shared<rosbag2_cpp::readers::SequentialReader>(
    default_storage_options_, converter_options,
    make_storage_mock(), converter_factory_, make_metadataio_mock());
  auto reader_ = std::make_unique<rosbag2_cpp::Reader>(std::move(sequential_reader));
  ...
  dynamic_cast<rosbag2_cpp::reader_interfaces::FilteredReaderInterface&>(
